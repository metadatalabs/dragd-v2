...
import { deleteSite, deployToIPFS, invalidateSiteBuildCache, updateSite, useSiteBuildByName } from '../DataProvider';
...
export default function PageSettingsModal({siteData, onComplete}) {
    ...
    {page == pages[0] && <>
        ...
        const DeployToIpfs = ({siteData}) => {
            ...
            const {data:buildData, isLoading:buildDataLoading} = useSiteBuildByName(siteData.siteName);
        ...
        {siteBuildData && <>
            ...
            <div tabIndex={0} className="collapse collapse-plus border border-base-300 bg-base-100 rounded-box"> 
                ...
                {siteBuildData.buildCIDs.map((cid, index) => <p className={`text-left p-1 ${index % 2 == 0? 'bg-base-100':'bg-base-200' }`} key={index}>{cid}</p>)}
                </div>
            </div>
            </>}
            <button onClick={()=>{
