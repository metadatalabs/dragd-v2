...
export default function VisibleCanvas(props) {
    ...
    useEffect(() => {
        setVisiblePageHeight((pageHeight * getMobileScaleRatio()) + (mode === EditorModes.EDIT ? 100 : 0));
    ...
    useEffect(() => {
        ...
        Object.values(items).forEach(element => {
            if(element.pos.x < bottomRight.x &&
    ...
    return <div style={{
        ...
        <div style={{ transform:`scale(${pageWidth? getMobileScaleRatio(): 1})` }}>
            ...
            {props.children}
            {isDragging && <SelectionBox topLeft={topLeft} bottomRight={bottomRight}/>}
            </div>
        </div>
    </div>
}
