...
export default function GenericDropdown(props){
    ...
    <div>
      ...
      <button onMouseDown={onClick} type="button" className="transition-all inline-flex w-full justify-center rounded-md border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 shadow-sm hover:bg-gray-50 hover:outline-none hover:ring-2 hover:ring-dragd hover:ring-offset-2 hover:ring-offset-gray-100" id="menu-button" aria-expanded="true" aria-haspopup="true">
        {label}
    ...
    {isActive && <div onMouseLeave={onMouseLeave} className="transition-all absolute right-0 z-10 mt-2 w-56 origin-top-right rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none" role="menu" aria-orientation="vertical" aria-labelledby="menu-button" tabindex="-1">
      <div className="max-h-80 overflow-auto p-1" role="none">
        {options?.map((item, index) => {
            return <div className="block" role="menuitem" tabindex="-1" id={`menu-item-${index}`}>{item}</div>
        })}
      </div>
    </div>}
  </div>  
